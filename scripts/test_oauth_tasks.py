...
async def run_callback_server(
    ...
    async def oauth_callback(request):
        ...
        return web.Response(
            text="<html><body><h1>❌ Код не получен</h1></body></html>",
            content_type="text/html",
        )
...
def get_tokens():
    ...
    if AIOHTTP_AVAILABLE:
        ...
        try:
            code = asyncio.run(
                run_callback_server(client_id, client_secret, scopes, port)
            )
            if code:
...
def main():
    ...
    parser.add_argument(
        "--get-tokens", action="store_true", help="Получить OAuth токены через браузер"
    )
    parser.add_argument(
        "--create-task", action="store_true", help="Создать тестовую задачу"
    )
    parser.add_argument(
        "--list-tasks", action="store_true", help="Показать список задач"
    )

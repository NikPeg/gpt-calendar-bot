...
class UpdateTaskCommand(TasksCommand):
    ...
    async def execute(self) -> str:
        ...
        if task:
            return f"✅ Задача обновлена успешно!\n\nID: {task_id}"
        return f"❌ Не удалось обновить задачу с ID {task_id}"
...
class DeleteTaskCommand(TasksCommand):
    ...
    async def execute(self) -> str:
        ...
        if success:
            return f"✅ Задача удалена успешно!\n\nID: {task_id}"
        return f"❌ Не удалось удалить задачу с ID {task_id}"
...
class CompleteTaskCommand(TasksCommand):
    ...
    async def execute(self) -> str:
        ...
        if task:
            return f"✅ Задача отмечена как выполненная!\n\nID: {task_id}"
        return f"❌ Не удалось завершить задачу с ID {task_id}"

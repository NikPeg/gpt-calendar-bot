...
async def get_llm_response(
    ...
    if conversation.oauth_access_token:
        calendar_tasks_info = (
            ...
            "7. ОБЯЗАТЕЛЬНО указывай ID событий и задач в ответе пользователю (например: 'Событие \"Встреча\" (ID: abc123) успешно создано')\n\n"
            "🎯 FLOW РАБОТЫ С API (КРИТИЧЕСКИ ВАЖНО!):\n"
            "1. НИКОГДА не говори пользователю что операция выполнена ПОКА НЕ ВЫЗВАЛ функцию!\n"
            "2. Сначала ВЫЗОВИ функцию (например create_calendar_event)\n"
            "3. ДОЖДИСЬ результата от API (функция вернет JSON с полями 'status' и 'message')\n"
            "4. Прочитай поле 'status' из результата:\n"
            "   - Если status='success': сообщи пользователю об успехе, используя message из ответа\n"
            "   - Если status='error': сообщи пользователю об ошибке, используя message из ответа\n"
            "5. ВАЖНО: НЕ придумывай результаты! Используй только реальные данные из ответа API!\n"
            "6. Пример правильного flow:\n"
            '   Пользователь: "Создай событие завтра"\n'
            "   Ты (про себя): Нужно вызвать create_calendar_event\n"
            "   [Вызов API]\n"
            '   API возвращает: {"status": "success", "event_id": "abc123", "message": "✅ Событие создано..."}\n'
            '   Ты пользователю: "Событие создано успешно! ID: abc123"\n\n'
